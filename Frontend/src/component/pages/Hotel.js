            ...
            {data.length>0?
                data.map((el,index)=>(
                    ...
                    <div className="row g-3">
                        ...
                        data-wow-delay="0.1s"
                        >
                        <Link to={`/rooms/${el._id}`} className=" text-capitalize position-relative d-block overflow-hidden" href="">
                            <img className="img-fluid w-100" src={BASE_URL+el.image} alt="" style={{height:"300px"}} />
                            <div className="bg-white text-danger fw-bold position-absolute top-0 start-0 m-3 py-1 px-2">
                            {el.name}
                            </div>
                            <Link to={`/rooms/${el._id}`}className="bg-white text-primary fw-bold position-absolute bottom-0 end-0 m-3 py-1 px-2">
                            View Details
                            </Link>
